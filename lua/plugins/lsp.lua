...
return {
    ...
    },
    {
        ...
        config = function()
            ...
            vim.api.nvim_create_autocmd("LspAttach", {
                group = vim.api.nvim_create_augroup("lspsaga-lsp-attach", { clear = true }),
                callback = function(event)
                    require("lspsaga").setup({})
                    ...
                    -- Rename variable
                    map("<leader>lra", ":Lspsaga rename<CR>", "[L]SP [R]ename [A]ll")

                    -- Symbol outline
                    -- https://nvimdev.github.io/lspsaga/outline/
                    map("<leader>lo", ":Lspsaga outline<CR>", "[L]SP [O]utline")

                    -- Code Action (normally `vim.lsp.buf.code_action`)
